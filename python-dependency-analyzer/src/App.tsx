    ...
    return (
      <div className="bg-white shadow p-4 mb-4">
        <div className="max-w-7xl mx-auto flex gap-4">
          <div className="px-3 py-2 rounded bg-gray-100">...</div>
        </div>
      </div>
    );
  }
      ...
      <Suspense fallback={
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-lg p-6">
            <p className="text-lg text-gray-600">{isLoading ? '...' : t.messages.loading}</p>
          </div>
        </div>
      }>
        <Routes>
          ...
          <Route path="/package-alternative" element={
            <React.Suspense fallback={<div>{isLoading ? '...' : t.messages.loading}</div>}>
              <PackageAlternative />
            </React.Suspense>
          } />
        </Routes>
